...
class Query<TM extends Record<string, unknown> = {}> {
    ...
    table<Name extends string>(tableMap: Name, tableName: string) {
      return new TableColumn<Name , TM>(this);
    }
}

class TableColumn<Name extends string, TM extends Record<string, unknown>> {
